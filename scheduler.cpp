...
AbstractTask* Scheduler::pop_task()
{
    refresh_queue();
...
unsigned int Scheduler::push_task(AbstractTask *task)
{
    std::lock_guard<std::mutex> guard(_access_to_queue);
...
unsigned int Scheduler::push_task(AbstractTask *task, int msec, TypeTask type)
{
...
void Scheduler::import_task(decltype (_queue_deffered_tasks)::iterator it)
{
    _access_to_queue.lock();
...
bool Scheduler::remove_task(unsigned int index)
{
...
Scheduler::Scheduler(unsigned int count_thread):
